...
class RepairOrphanedSubshare implements IRepairStep {
	...
	public function __construct(IDBConnection $connection) {
		...
		$this->missingParents->select('parent')
			->from('share');
		$this->missingParents->where($this->missingParents->expr()->notIn('parent', $this->missingParents->createFunction($qb->getSQL())));
	...
	public function run(IOutput $output) {
		...
		if (count($deleteParents) > 0) {
			foreach ($deleteParents as $deleteParent) {
				$this->deleteOrphanReshares->setParameter('parentId', $deleteParent)->execute();
			}
		}
	}
}
