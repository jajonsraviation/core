...
trait Provisioning {
	...
	public function userShouldExist($user) {
		PHPUnit_Framework_Assert::assertTrue($this->userExists($user));
		$this->addUserToCreatedUsersList($user, $this->regularUserPassword);
	}
	 ...
	 * @return void
	 */
	public function deleteTheUserUsingTheAPI($username) {
		...
		if ($this->theUserShouldHaveBeenCreated($username) && ($this->response->getStatusCode() !== 200)) {
			error_log(
